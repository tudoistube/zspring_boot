...
public class ReadingListController {
	...
	//...S.222p.사용자 정의 메트릭 엔드포인트 사용을 위한 게이지와 카운터 변수 선언.
	private CounterService counterService;
	private GaugeService gauageService;
	//...E.222p.사용자 정의 메트릭 엔드포인트 사용을 위한 게이지와 카운터 변수 선언.
	...
	public ReadingListController(ReadingListJpaRepository readingListJpaRepository,
								 ...
								 CounterService counterService, GaugeService gauageService) { //...222p.added.
		...
		this.counterService = counterService; //...222p.added.
		this.gauageService = gauageService; //...222p.added.
	}	
	...
	public String addToReadingList(READER_DTO reader, BookDto book){
		...
		//...S.222p.사용자 정의 메트릭 엔드포인트 사용을 위한 게이지와 카운터 변수 사용.
		//...addToReadingList() 메서드가 요청을 처리할 때 마다 카운트하고 기록함.
		//...books.saved 메트릭 증가.
		this.counterService.increment("zbooks.saved"); 
		//...books.last.saved 기록.
		this.gauageService.submit("zbooks.last.saved", System.currentTimeMillis()); 
		//...E.222p.사용자 정의 메트릭 엔드포인트 사용을 위한 게이지와 카운터 변수 사용.	
