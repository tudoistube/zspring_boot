...
/*
 * ...079p.Fetch Join을 이용한 게시글과 댓글의 숫자에 대한 처리 : 
			예) 흔히 게시물의 리스트 화면에서는 게시물의 제목 옆에 댓글의 숫자를 보여줌. 

			문제는 현재의 Board엔티티 클래스는 Reply에 대한 참조가 전혀 없기 때문에 
			댓글의 숫자를 참조할 수 있는 방법이 전혀 없다는 것입니다. 
			이를 해결하려면 ‘Fetch Join’이라는 JPQL을 이용해야만 합니다. 
			
			Fetch Join 이란?
			Fetch Join은 JPQL 에서 SQL의 Join기능과 유사한 처리를 할 수 있는 기능임. 
			이를 이용하면 주로 다음과 같은 상황을 처리할 수 있음. 
			대상 엔티티가 참조하지 않는 다른 엔티티를 필요한 순간에만 참조하는 경우 
			연관된 엔티티의 컬렉션들을 처리해야 하는 경우 
			Fetch Join의 문법적인 구성은 다음과 같이 작성됨. 
			
			페치 조인 ::= [LEFT [OUTER] | INNER] JOIN FETCH 조인경로
			
			예를 들어 현재 200번의 게시물은 여러 개의 댓글을 가지고 있기 때문에 
			게시물 조회시에 이를 같이 사용해야 한다면 다음과 같이 작성할 수 있습니다.
			 
			Fetch Join은 Outer join을 사용할 수 있고, count( ), sum( )등의 기능을 같이 사용해서 
			필요한 집합함수의 기능들을 사용할 수 있음. 
			다만 Fetch Join으로 하나의 엔티티 외에 정보를 가져오는 경우에는 List<Object[ ]>을 
			이용하는 것이 편리함. 
			
			Fetch Join을 이용하면 참조가 없는 엔티티에 대한 추가적인 작업이 가능하기 때문에 
			실제 개발에서도 많이 사용됨. 
 */
	@Query("select b, count(r)"
			+ " from Board b LEFT OUTER JOIN Reply r "
			+ " ON b.bno = r.board where b.bno = ?1")
	public List<Object[]> findBoardWithReply(Long bno);
